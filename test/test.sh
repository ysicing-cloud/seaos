...
remotecmd "wget https://github.com/ysicing-cloud/sealos/releases/download/v3.0.1/sealos && chmod +x sealos && mv sealos /usr/bin"
...
remotecmd "sealos init --master $master0 --master $master1 --master $master2 \
    --node $node --passwd Fanux#123 --version v$version --pkg-url $pkgurl"

echo "wait for everything ok"
sleep 40
./sshcmd --passwd Fanux#123 --host $master0FIP --cmd "kubectl get node && kubectl get pod --all-namespaces && kubectl get cs"
